...
module.exports = class Help extends Base {
    ...
    async run(msg, args, data){
        ...
        // const buffer = await (await (fetch(`${this.bot.baseApiUrl}/canvas/circle?key=${this.bot.config.apiKey}&imgUrl=${imgUrl}`))).buffer();
        let buffer;
        try{
            buffer = await (await (fetch(`${this.bot.baseApiUrl}/canvas/circle?key=${this.bot.config.apiKey}&imgUrl=${imgUrl}`))).buffer()
        } catch (err) {
            console.log(`${__filename} - ${err}`)
            return msg.channel.createMessage('An error happened, Joined my support server for help! ||<https://monkedev.com/r/discord>||');
        }

        msg.channel.createMessage('', {file: buffer, name: `circle.png`});
    };
};